...
const WorkForm = () => {
  ...
  const {
    ...
    setValue,
    formState: { isValid },
  } = useForm<z.infer<typeof workSchema>>({
    resolver: zodResolver(workSchema),
    defaultValues: {
      works: [
        { position: "", workCompany: "", start: "", end: "", summery: "" },
      ],
    },
  });
    ...
    <div className="min-h-[70%] bg-white border-1 border-gray-300 border-t-blue-400 border-t-4 dark:bg-neutral-800 shadow-lg rounded-lg p-6 lg:p-10 w-full">
      ...
      >
          ...
          {fields?.map((_, index) => (
            <div key={index} className="mb-1 border-1 rounded">
              <div className="flex flex-col justify-between p-5">
                ...
                <div className="flex flex-col p-1 w-full"></div>
                <div className="flex">
                ...
                <div className="w-full">
                  ...
                  {loading ? (
                    <div className="w-full min-h-20 rounded-xs border-1 p-5">
                    ...
                    <textarea
                      className="min-h-20 font-medium py-3 px-4 block w-full border rounded-sm text-sm focus:border-green-400 focus:ring-1 focus:ring-green-400 focus:outline-none dark:text-white dark:placeholder-gray-400"
                      placeholder="Describe your key responsiblities and any notable achievements"
                      {...register(`works.${index}.summery`, {
                        required: true,
                      })}
                    />
                  )}
                  <span className="text-xs text-gray-500 flex items-center">
                    <Lightbulb size={12} />
                    If unsure what to write, let AI handle it!
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="flex justify-between my-10">
          <div className="flex gap-2">
            <Button
              ...
              type="button"
              className="text-blue-500 border-1 bg-white border-gray-200 hover:bg-inherit"
              size={"lg"}
            >
            ...
            <Button
              type="button"
              onClick={() => remove(-1)}
              className="text-red-500 border-1 bg-white border-gray-200 hover:bg-inherit"
              size={"lg"}
            >
              - Remove
            </Button>
        ...
        <div className="flex justify-end">
          <Button
            className="bg-blue-400 text-sm font-medium text-end"
            size={"sm"}
            onClick={() => updateCurrentSection("education")}
          >
            <MoveRight /> Continue
          </Button>
        </div>
